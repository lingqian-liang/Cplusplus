...
vector<double> Compute_AAR(map<string,Stock> &group, const int &snum, const int &stime){
    ...
    srand((unsigned)time( NULL ));//sign up the seed
    ...
    for (int t=0; t<stime; t++){
        ...
        for (int s=0;s<snum;s++){
            ...
            if (s==0){
                AAR = group[getkey].get_return_val()-Mkt_s;//operator overloading
            }
            else{
                AR_temp= group[getkey].get_return_val()-Mkt_s;//operator overloading
                AAR = AAR+AR_temp;
            }
            getkey.clear();
        }
        ...
        {//retrive stock from database
            for (unsigned int i = 0; i < DataBase_div.size(); i++) {
                ...
                std::transform(
                               ...
                               (int (*)(int))std::toupper //for overloading. because std also has a function called toupper
                               );
